...
int main()
{
    ...
    sem_args.lock = sem_open ( "lock", O_CREAT | O_EXCL, 00666, 0 );
    if ( sem_args.lock == SEM_FAILED )
    {
        printf ( "semaphore 'lock' create error: %s\n", strerror ( errno ) );
        return -1;
    }
    ...
    for ( int i = 0; i < NUM_SUPPLIES; i++ )
    {
        ...
        //  wake lock and block 
        if ( sem_post ( sem_args.lock ) )
            printf ( "WARNING: sem_wait(lock) error: %s\n", strerror ( errno ) );
    ...
    for ( int i = 0; i < ITERATIONS; i++ )
    {
        ...
        int rand_num = rand() % (NUM_SUPPLIES);
        sem_args.found = 0;

        // place item on table
        for ( int i = 0; i < NUM_SUPPLIES; i++ )
            if ( i != rand_num )
                sem_args.table[rand_num] = 1;
        printf ( "\nAgent placed two unknown items on the table...\n" );
    ...
    //  send terminate kill_sig to smoker processess
    sem_args.kill_sig= -1;
    ...
    if ( sem_unlink ( "lock" ) )
        printf ( "WARNING: semaphore 'lock' destroy error: %s\n", strerror(errno) );
    return 0;
}
...
void *smoke( void *arg )
{
    ...
    // pick up items
    while ( sem_args->kill_sig != -1 )
    {
        if ( sem_args->found == 0 )
        {
            if ( sem_wait ( sem_args->lock ) )
                printf ( "WARNING: sem_wait(lock) error: %s\n", strerror(errno) );

            if ( sem_args->table[id] == 1 )
            {
                //  flag found
                sem_args->found = 1;
                for ( int i = 0; i < NUM_SUPPLIES; i++ )
                ...
                printf ( "\t...the smoker w/ %s checked the table and was unable "
                         "to make anything...\n", items[id] ); 
        }
